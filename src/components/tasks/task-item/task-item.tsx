...
const TaskItem = ({tasks, board, showCompleted, updateTaskElement, setIsEditTaskOpen, setSelectedTask} : TaskItemProps) => {
  ...
  return (
    <>
    {
      tasks.filter(task => task.boardType === board && (!showCompleted || !task.completedStatus))
        .map(task => (
            ...
            </div>
            {
              (
                <div className='task__bottom'>
                  ...
                  {
                    task.priority &&
                    task.priority !== Object.values(Priority).length ? (
                      <span className='task__priority-item' title='TaskItem priority'>
                        ...
                        <span className='priority__title'>
                          {` ${Object.values(Priority)[task.priority - 1]} priority`}</span>
                        </span>
                    ) : null
                  }
                </div>
