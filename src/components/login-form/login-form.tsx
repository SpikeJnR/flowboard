  ...
  return (
    <div className='login__form'>
      ...
      <form className='login__form-group' onSubmit={handleSubmit}>
        ...
        <input className='password__input input'  type={showPassword ? 'text' : 'password'} placeholder='Enter the password' id='password__input' onChange={(evt) => setPassword(evt.target.value)} required pattern={'^[A-Za-z0-9]{8,30}$'}  title="Пароль должен иметь длинну от 8 до 30 символов. И содержать только латинские цифры и букы"/>
        <label className='password__label' htmlFor='password__input'>Password</label>

        <button className='password__toggle' type='button' aria-label='show password' onClick={() => setShowPassword(!showPassword)}>
          <img className='password__icon'  src={showPassword ? './public/images/password-visibility.svg' : './public/images/password-visibility-off.svg'} width='24px' height='24px' id="password__icon"/>
        </button>
